...
;; Use `declare` to reference functions before they're defined.
...
(defn print-board
  [board]
  (doseq [rownumbr (range 1 (inc (:rows board)))]
    (println (render-row board rownumbr))))
...
(defn get-input
  "wait for the user to enter text and hit enter, then clean the input"
  ([] (get-input nil))
  ([default]
...
(defn prompt-move
  "read the player's input and act on it"
  [board]
  (println "\nHere's your board:")
  (print-board board)
